  ...
  var sentence = "去 不要回來 舔共藝人 賣國求榮 賤畜 習近平的形狀 破腦麻仔 陪睡 噓 噁 討厭 胖 蔣開穴氣到棺材板跳起來 槍斃 沒存在感 米田共 垃圾 爛咖 煩";
  ...
  function draw() {
    ...
    for (let i = 0; i < particles.length; i+=50) {
      ...
      for (let j = 0; j < 3; j++) {
        textColor[j] += (desiredColor[j] - textColor[j]) ;
      }
      ...
      velocity.add(p5.Vector.sub(desiredPosition, currentPosition).mult(0.03));
